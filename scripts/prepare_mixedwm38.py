...
입력: E:/data/images/mixedwm38/Wafer_Map_Datasets.npz  (arr_0 maps, arr_1 8-dim multi-hot)
...
출력: <out>/all/<class>/<idx>.png + manifest.csv (기본 E:/data/images/mixedwm38/rendered, --out 로 override 가능
      — 260726 정책: 데이터셋은 E: 단일 마스터 폴더에 전체 렌더하고 subset 은 manifest 로 선택)
...
NPZ = Path("E:/data/images/mixedwm38/Wafer_Map_Datasets.npz")
OUT = Path("E:/data/images/mixedwm38/rendered")
...
def main():
    ...
    ap.add_argument("--npz", type=str, default=str(NPZ), help="Wafer_Map_Datasets.npz path")
    ap.add_argument("--out", type=str, default=str(OUT), help="output root (all/<class>/*.png 생성)")
    args = ap.parse_args()
