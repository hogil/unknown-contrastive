...
def render(class_name, object_name, seed):
    ...
    # 260527 patch (사용자 명시 v8): 2 stream noise — ㅡ/ㅣ short line + scratch 1줄 옅게.

    # === Stream A: scratch line ★ 1개만 ★ (alpha_scratch 의 5-10 line 중 1줄만 추출) ===
    try:
        from _synth_chip import _scratch_line as _scratch_line_fn, CHIP_BASE_ALPHA, CHIP as _CS_CHIP
        _HAVE_SC = True
    except Exception:
        _HAVE_SC = False

    def _alpha_scratch_single(rng_):
        """alpha_scratch 1 line — y 마다 좌/우 음영 strong 독립.
        _scratch_line 은 right-smear only. 두 호출 (right + flipped left) 만들고,
        y 위치마다 어느쪽 사용할지 smooth random direction 으로 결정.
        → 같은 line 위 y=20% 왼쪽 강, y=40% 오른쪽 강, ... independent."""
        weak_only = rng_.random() < 0.18
        cx_one = float(rng_.uniform(15, 185))
        # 음영 최고 너비 줄임 (사용자 명시): max 50/60 → 42/45, 최소 35 그대로.
        smear1 = float(rng_.uniform(35.0, 42.0)) if weak_only else float(rng_.uniform(35.0, 45.0))
        smear2 = float(rng_.uniform(35.0, 42.0)) if weak_only else float(rng_.uniform(35.0, 45.0))
        # right-smear 와 left-smear 각각 생성 (다른 random — 모양 다름)
        a_right = _scratch_line_fn(rng_, cx_one, weak_only, smear1)
        cx_flipped = float(_CS_CHIP - 1 - cx_one)
        a_left = _scratch_line_fn(rng_, cx_flipped, weak_only, smear2)[:, ::-1]

        # y 마다 좌/우 strong 가중치 결정 — smooth random (line 따라 자연 transition).
        # weight_right ∈ [0, 1] : 1 = right 우세, 0 = left 우세
        # 두 weight 합 = 1 으로 강제 (한 쪽이 강할 때 다른쪽 약함)
        from _synth_chip import _along_smooth_range_1d as _smooth_y
        w_right = _smooth_y(rng_, _CS_CHIP, 0.0, 1.0).astype(np.float32)        # (CHIP,) 0~1 smooth
        w_right = w_right[:, None]                                                # (CHIP, 1)
        w_left = 1.0 - w_right
        # y 별 가중 합 + baseline (둘 다 살짝 보이게 baseline 작게)
        a = (a_right * w_right + a_left * w_left).astype(np.float32)
        a = np.maximum(a, CHIP_BASE_ALPHA)
        return a
    ...
    if _HAVE_SC:
        ...
        for _ in range(n_sc):
            ...
            cy = int(rng.integers(patch_size, SIZE - patch_size))
            cx = int(rng.integers(patch_size, SIZE - patch_size))
            if not inside_pix[cy, cx]:
                continue
            y0 = cy - patch_size // 2; y1 = y0 + patch_size
            x0 = cx - patch_size // 2; x1 = x0 + patch_size
            if y0 < 0 or x0 < 0 or y1 > SIZE or x1 > SIZE:
                continue

            # ★ alpha_scratch 그대로 (line 1개만) + chip_synth 의 grade 만들기 알고리즘
            alpha = _alpha_scratch_single(rng).astype(np.float32)                  # 200×200, 1 line

            # 약 30% 확률 — line 살짝 구부러지게 (column 별 row shift, sinusoidal)
            if rng.random() < 0.30:
                amp = float(rng.uniform(2.0, 6.0))
                period = float(rng.uniform(60.0, 140.0))
                _yy = np.arange(_CS_CHIP)
                shifts = (amp * np.sin(2 * np.pi * _yy / period)).astype(np.int32)
                curved = np.zeros_like(alpha)
                for _y in range(_CS_CHIP):
            ...
            grade_map = np.where(is_defect, defect_grade, np.uint8(0)).astype(np.uint8)

            # resize + rotate (grade 정수 → NEAREST 으로 보존)
            grade_img = _PILI.fromarray(grade_map)
            grade_img = grade_img.resize((patch_size, patch_size), _PILI.NEAREST)
            grade_img = grade_img.rotate(float(rng.uniform(0.0, 360.0)),
                                         resample=_PILI.NEAREST, fillcolor=0)
            grade_r = np.asarray(grade_img)
            sub = canvas[y0:y1, x0:x1]
            inside_sub = inside_pix[y0:y1, x0:x1]
            write = (grade_r > 0) & inside_sub & (sub == 0)
            canvas[y0:y1, x0:x1] = np.where(write, grade_r, sub)
    ...
    for _ in range(n_lines):
        ...
        if not inside_pix[cy, cx]:
            continue
        length = int(rng.integers(8, 80))                                              # 10~80 px 다양 (chip 보다 작음)
        ang = float(rng.choice([0.0, np.pi / 2]))                                      # ★ ㅡ (horizontal) 또는 ㅣ (vertical) 만
        thickness = int(rng.choice([1, 1, 2, 2, 3], p=[0.35, 0.25, 0.2, 0.15, 0.05]))  # 1 dominant
        grade_main = 7   # palette idx 7 = chip7 = #000000 (검은색) — 사용자 명시
        taper = bool(rng.random() < 0.5)                                               # 50% 양 끝 fade
        dy = np.sin(ang); dx = np.cos(ang)
        n_steps = max(2, length * 2)
        for t_idx, t in enumerate(np.linspace(-length / 2, length / 2, n_steps)):
            # taper: 양 끝 의 line probability 줄임 (음영 효과)
            if taper:
                t_norm = abs(t) / (length / 2 + 1e-6)
                if rng.random() < t_norm * 0.7:
                    continue
            py = int(round(cy + t * dy))
            px = int(round(cx + t * dx))
            for ty_off in range(-(thickness - 1) // 2, thickness // 2 + 1):
                for tx_off in range(-(thickness - 1) // 2, thickness // 2 + 1):
                    yy_p, xx_p = py + ty_off, px + tx_off
