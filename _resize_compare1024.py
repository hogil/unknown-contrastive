"""1024×1024 cache 후보 6개 방식 시각 비교.
...
방식:
  ...
  4) LANCZOS
  5) BOX (= AREA, 영역 평균)
  6) chip-aware (외곽 1px = NEAREST stride, 내부 30×30 BOX 평균)

출력: _resize_compare1024/<class>/{method}.png + grid.png + zoom_grid.png
"""
...
CHIP_PX = 200
OUT_SIZE = 1024
CHIP_OUT = OUT_SIZE // CHIP_GRID                 # 32
VIEW_SIZE = 1024                                  # raw 1024 그대로 view
...
def process_one(src_path: Path, out_dir: Path):
    ...
    # ===== Full wafer grid (1024 view) =====
    labeled = []
    for name, im in methods.items():
        labeled.append(label_image(im, f"1024 {name}"))
    ...
    for name, im in methods.items():
        ...
        zoom_labeled.append(label_image(view, f"1024 {name}  (zoom 6×6 chip)"))
...
def main():
    ...
    base_out = Path('D:/project/unknown-contrastive/_resize_compare1024')
