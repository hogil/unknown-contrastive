...
"""Inference CLI for the unknown-contrastive project.

Loads a trained checkpoint (``final_infer.pt``), re-uses the transform and
model from ``contrastive.py``, embeds every image under ``--images-dir`` and
routes it to ``cluster_XXX/`` (or ``unknown/``) under ``--out-dir``.
