...
CFG: Dict[str, Any] = {
    ...
    "MIN_SAMPLES": 1,                 # 코어 포인트 기준(차원/밀도 민감)
    "CLUSTER_SELECTION_METHOD": "eom",  # 'leaf'는 세분화, 'eom'은 굵게
    "CLUSTER_SELECTION_EPSILON": 0.0,   # 클러스터 선택 스무딩(크면 합치기 경향)
...
def main():
    ...
    # Data
    ds_u = ImageFolder(CFG["UNKNOWN_DIR"])               # 전수 임베딩/클러스터링 대상
    ds_t = ImageFolder(CFG["TRAIN_DIR"])                 # 학습 대상(작은 셋)
