...
-- REVISION + COMPOSITE-INLINED VARIANT (training improvements + composite inlined)

Builds on contrastive_com.py (composite inlined) with learning-pipeline
revisions for robustness and recoverability:

  * save_embedding_artifacts() — persists emb.npy + cluster_labels.npy +
    files.txt + kept_labels.txt after HDBSCAN so composite/predict/eval
    can be replayed without re-embedding when the training tail crashes.

All core training logic (backbone, InfoNCE, local loss, HDBSCAN, centroid
save, inlined composite) is unchanged from contrastive_com.py.
...
- Saves:
  ...
  - run_dir/cluster_summary/composite/cluster_XXX_composite.png (INLINED)
  - emb.npy, cluster_labels.npy, files.txt, kept_labels.txt (REVISION)
  - ignored_samples/
...
# =========================================================
# Revision: embedding artifact persistence for downstream replay
# =========================================================
def save_embedding_artifacts(
    ...
    Produces four files in ``run_dir``:
        emb.npy             — L2-normalized embedding array (N, D)
        cluster_labels.npy  — HDBSCAN cluster id per sample (N,)
        files.txt           — image path per sample, parallel to emb
        kept_labels.txt     — cluster ids that passed the KEEP filter

    Matches the filenames recovery scripts and composite CLI expect, so a
    finished run can be replayed for composite/predict/eval without
    re-embedding.
    """
...
# =========================================================
# Inlined cluster composite (was: cluster_composite.py)
# =========================================================
def _composite_l2_normalize(x: np.ndarray, axis: int = -1, eps: float = 1e-12) -> np.ndarray:
    n = np.linalg.norm(x, axis=axis, keepdims=True)
    return x / np.maximum(n, eps)


def _composite_select_medoid_indices(emb: np.ndarray, member_idxs: np.ndarray, n: int) -> np.ndarray:
    """Indices (into full ``emb``) of top-N cosine-closest members to cluster mean."""
    if member_idxs.size == 0:
        return member_idxs
    members = emb[member_idxs]
    mean = members.mean(axis=0, keepdims=True)
    mean = _composite_l2_normalize(mean)[0]
    mem_norm = _composite_l2_normalize(members, axis=-1)
...
def generate_cluster_composites(
    ...
    """Generate one composite PNG per KEPT cluster (inlined from cluster_composite.py)."""
    _ = overlay_dir
    ...
    for lab in sorted(int(x) for x in kept_labels):
        ...
        if member_idxs.size == 0:
            if logger is not None:
                logger.warning(f"[cluster_composite] cluster {lab:03d} has no members; skipped.")
            continue

        top_idxs = _composite_select_medoid_indices(emb, member_idxs, n_per_cluster)
        ...
        try:
            ...
            if logger is not None:
                logger.warning(f"[cluster_composite] stack failed for cluster {lab:03d}: {e}")
            continue

        out_path = out_dir / f"cluster_{lab:03d}_composite.png"
        try:
            info = render_composite_png(stacked, out_path, title=f"cluster_{lab:03d} (n={stacked.shape[0]})")
        except Exception as e:
            if logger is not None:
                logger.warning(f"[cluster_composite] render failed for cluster {lab:03d}: {e}")
            continue

        if logger is not None:
            logger.info(f"[cluster_composite] cluster {lab:03d}: n={info['image_count']} -> {out_path.name}")
...
def main():
    ...
    # Revision: persist embedding + clustering artifacts so composite/predict/eval
    ...
    # Generate cluster composite maps (KEPT cluster only, inlined above)
    try:
