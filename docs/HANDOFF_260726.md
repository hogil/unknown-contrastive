...
**다이얼 정정 후** (mcs20/ms5/leaf — bootstrap 안정성이 k 없이 고른 값):
...
★ **260727 폐기: k 기반 다이얼 규칙은 존재하지 않는다.** k 는 모른다.
다이얼은 **bootstrap 군집 안정성 최대**로 다이얼을 고른다 (라벨·k 미사용, `deploy/_site_common.pick_dial_by_stability()`). 여러 다이얼에서 순위가 유지되는지도 확인한다 — 한 지점에서만 이기면 못 믿는다.
아래 표는 **폐기된 규칙의 흔적**이며 참고용도 아니다:
...
2. **k 는 받지 않는다** — 모르는 게 전제. 다이얼은 min_cluster_size 운영값 또는 안정성 자동선택.
   다이얼은 bootstrap 안정성으로 고른다 (k 미사용)
