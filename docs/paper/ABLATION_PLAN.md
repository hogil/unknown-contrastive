...
> Historical note: the `P1 cap` column below was generated by the old non-noise-presence rule
> (`n_clusters >= 1` for a class). It is retained as a historical record, but is not comparable
> to the current canonical P1, which requires the class to be a cluster's dominant/main class.
