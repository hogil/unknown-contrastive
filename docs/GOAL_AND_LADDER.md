...
**대안 = 기하 규칙 `mcs ≈ n/k 의 10%`** (pool 6개 검증) — 단 **k̂ 를 알아야 한다**
→ **사내 배포 요구사항: 예상 불량 종수를 현업에서 받아야 한다.** 그 다음 `n/(15k)`~`n/(8k)` 만 좁게 스윕.
...
2. **예상 불량 종수(k̂)를 현업에서 받는다** — 다이얼을 라벨 없이 못 고르므로
...
**확정 전 필수 체크**
1. 각 arm 을 **자기 최적점**에서 비교. 비교 영역은 **k ≈ 실제 클래스 수** (ARI 최대화 = 병합 치팅)
2. 다이얼을 **pool 기하**에 맞춤 (`mcs ≈ n/k·0.1`)
3. 대조군 = **z0, 용량 매칭**
